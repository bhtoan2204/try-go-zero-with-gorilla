...
func (c *cache) ZRemRangeByScore(ctx context.Context, key string, min float64, max float64) error {
	...
	if err != nil {
		return fmt.Errorf("zremrangebyscore with key=%s, min=%v and max=%v to redis failed, err=%w", key, min, max, err)
	}
	return nil
}
...
func (c *cache) ZCard(ctx context.Context, key string) (int64, error) {
	...
	if err != nil {
		return -1, fmt.Errorf("zcard with key=%s to redis failed, err=%w", key, err)
	}
	return val, nil
}
